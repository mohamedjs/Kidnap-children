        ...
        <div class="row">
          <div class="col-lg-8">
            ...
            <transition name="fade">
              ...
              <div class="loading_data" v-if="loading_posts_result">
                <span class="fa fa-spinner fa-spin"></span> Search About Posts Match Your added post
              </div>
          ...
          <div class="col-lg-4">
            <div class="search-result-side-menu-block filter-by">
              <p>Filter By</p>
              <form @submit.prevent="filter_post()">
                <ul class="filter-search list-unstyled">
                  <li>
                    <div class="select-city-group">
                      <label class="icon-box" for="select_area"></label>
                      <select class="custome-select-city" style="position: relative;" v-model="filter.gender" placeholder="Gander">
                        <option value="">Select Your Area</option>
                        <option value="1">Male</option>
                        <option value="2">Female</option>
                      </select>
                  ...
                  <li>
                    <label class="age-label">Range of Age</label>
                    <div class="row">
                      <div class="col-6">
                        <input class="age-range" v-model="filter.from" type="text" placeholder="From">
                      </div>
                      <div class="col-6">
                        <input class="age-range" v-model="filter.to" type="text" placeholder="To">
                      </div>
                    </div>
                  </li>
                </ul>
                <input class="apply-filter" type="submit" value="Apply">
              </form>
            </div>
          </div>
        </div>
      ...
      components: { carousel , 'vue-load-image': VueLoadImage },
       ...
       methods: {
         ...
         filter_post(){
           ...
           this.loading = true
           this.posts = []
           axios.get(base_url+'/posts', {params:_this.filter} )
         ...
         create_post(){
           this.posts = [];
           this.loading_posts_result = true
           var _this = this
             ...
             .then(function (response){
               console.log(response);
                  if(response.data.results.length > 0){
                    ...
                    _this.loading_posts_result = false
                    _this.next_url = null
                    alert('we found '+response.data.results.length +' posts match your posts')
                  }
                  else{
                  _this.loading_posts_result = false
                  _this.get_posts()
                   _this.data = new FormData()
                   //_this.post = ''
                   alert('there are no match result and we get all posts')
                 }
               }).catch(
                 error =>{
                   // if (error.response.status == 422){
       ...
       created() {
        ...
        // Detect when scrolled to bottom.
         window.addEventListener('scroll', e => {
           ...
           if((bottomOfPage || pageHeight < visible) && this.next_url && !this.loading_posts_result){
             this.load_posts();
           }
         });
         this.load_posts();
